...
export default function ChartSection({ data, chartType, setChartType, timeFilter, setTimeFilter }) {
      ...
      case 'area':
        return (
          <LineChart
            {...commonLineProps}
            series={[
              { data: responseData, label: t('api.response'), color: theme === "dark" ? 'text-slate-300': 'text-white-700' , area: true },
              { data: errorData, label: t('api.error'), color: theme === "dark" ? 'text-slate-300': 'text-white-700', area: true },
            ]}
          />
        );
      case 'bar':
  ...
  return (
    <CustomDiv type="foreground" className="lg:col-span-2 rounded p-6 border border-[#3B5B75]">
      <div className="flex items-center justify-between mb-6">
        <h2 className={"text-lg font-semibold text-white" + ( theme === 'dark' ? 'text-slate-300' : 'text-white-700' )}>{t('api.chart.response_errors')}</h2>
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-4 text-xs">
            <div className="flex items-center"><div className="w-3 h-0.5 bg-blue-400 mr-2"></div><span className={"" + ( theme === 'dark' ? 'text-slate-300' : 'text-white-700' )}>Response</span></div>
            <div className="flex items-center"><div className="w-3 h-0.5 bg-green-400 mr-2"></div><span className={"" + ( theme === 'dark' ? 'text-slate-300' : 'text-white-700' )}>Error</span></div>
          </div>
        </div>
      </div>
