...
export default function LogsTable({ filteredLogs, filterColumn, setFilterColumn, filterValue, setFilterValue, getStatusColor, getMethodColor }) {
  ...
  return (
    <CustomDiv type="foreground" className="rounded p-6 mt-6 border border-[#3B5B75]">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs">
          <thead>
            <tr className={"border-b border-[#3B5B75]" + ( theme === 'dark' ? 'text-slate-300' : 'text-white-700' )}>
          ...
          <tbody>
            {filteredLogs.map((log, i) => (
              <tr key={i} className="border-b border-[#3B5B75] hover:bg-[#0E1A3D]/50 transition-colors">
                <td className={"py-2 " + ( theme === 'dark' ? 'text-slate-300' : 'text-white-700' )}>{log.timestamp}</td>
                <td className="py-2"><span className={`px-2 py-1 rounded text-xs text-white ${getMethodColor(log.method)}`}>{log.method}</span></td>
                <td className={"py-2 font-mono" + ( theme === 'dark' ? 'text-slate-300' : 'text-white-700' )}>{log.endpoint}</td>
                <td className={`py-2 ${getStatusColor(log.status)}`}>{log.status}</td>
                <td className={"py-2" + ( theme === 'dark' ? 'text-slate-300' : 'text-white-700' )}>{log.responseTime}</td>
              </tr>
            ))}
