...
export default function MethodsCard({ methodData }) {
  const { t } = useTranslation();
  const theme = useTheme();
  return (
    <CustomDiv type="foreground" className="p-6 rounded shadow-lg border border-[#3B5B75]">
      <div className="flex items-center justify-between mb-6">
        <h3 className={"text-sm font-semibold" + ( theme === 'dark' ? 'text-slate-300' : 'text-white-700' )}>{t('api.http_methods')}</h3>
        ...
        {methodData.map(item => (
          <div key={item.method} className="flex items-center justify-between py-2">
            <div className="flex items-center space-x-3">
              <span className={"text-sm font-mono" + ( theme === 'dark' ? 'text-slate-300' : 'text-white-700' )}>{item.method}</span>
