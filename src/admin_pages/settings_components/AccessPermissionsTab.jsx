...
export default function AccessPermissionsTab() {
  ...
  const updateUserRole = async (userId, newRole) => {
    try {
      ...
      setMessage({
        type: "success",
        text: "Role do usuário atualizado com sucesso!",
      });
  ...
  const toggleUserStatus = async (userId) => {
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: "Status do usuário atualizado com sucesso!",
      });
      setTimeout(() => setMessage({ type: "", text: "" }), 3000);
    } catch (error) {
                  ...
                  >
                    ...
                    <td className="p-4">
                      <select
                        ...
                        className={`px-3 py-1 rounded-full text-white text-xs font-medium ${getRoleColor(
                          user.role
                        )} focus:outline-none`}
                      >
                        {roles.map((role) => (
