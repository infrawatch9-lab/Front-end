  ...
  return (
    <CustomDiv className="bg-[#0B1440] p-6 rounded-xl shadow-2xl border border-slate-700/50 backdrop-blur-sm">
      <CustomDiv className="flex items-center justify-between mb-6">
        <h2 className={"text-xl font-bold text-white flex items-center gap-3 "  + (theme == 'dark' ? " text-colors-light " : " text-colors-dark ")}>
      ...
      <CustomDiv className="space-y-6">
        {systemStats.map((stat) => {
          ...
          return (
            <CustomDiv key={stat.id} className="group hover:scale-[1.02] transition-all duration-300">
              <CustomDiv className="flex items-center justify-between mb-3">
                <CustomDiv className="flex items-center gap-3">
                  <CustomDiv className={`p-2 rounded-lg ${stat.bgColor} group-hover:scale-110 transition-transform duration-300`}>
                    <Icon className={`w-5 h-5 ${stat.iconColor}`} />
                  </CustomDiv>
                  <CustomDiv className="flex flex-col">
                    <span className={"text-gray-200 font-medium text-sm sm:text-base " + (theme == 'dark' ? " text-colors-light " : " text-colors-dark ") }>{stat.label}</span>
                    <CustomDiv className={"flex items-center gap-2 mt-1 "  + (theme == 'dark' ? " text-colors-light " : " text-colors-dark ")}>
                      <TrendingUp className={`w-3 h-3 ${stat.trend === 'up' ? 'text-green-400' : 'text-red-400'} ${stat.trend === 'down' ? 'rotate-180' : ''}`} />
                      <span className={"text-xs text-gray-400 "  + (theme == 'dark' ? " text-colors-light " : " text-colors-dark ")}>
              ...
              <CustomDiv className="relative">
                <CustomDiv type="background" className="w-full bg-[#1E2A5C]/60 rounded-full h-3 overflow-hidden shadow-inner">
                  <div
                ...
                <CustomDiv className="flex justify-between mt-1 text-xs text-gray-500">
                  ...
                  <span className={ (theme == 'dark' ? " text-colors-light " : " text-colors-dark ")}>{stat.total}{stat.suffix || ''}</span>
                </CustomDiv>
              </CustomDiv>
            </CustomDiv>
          );
      ...
      <CustomDiv className="mt-6 pt-4 border-t border-slate-700/50">
        <CustomDiv className="flex items-center justify-between">
          <span className={"text-gray-400 text-sm " + (theme == 'dark' ? " text-colors-light " : " text-colors-dark ") }>{t('system_overview.general_status')}</span>
