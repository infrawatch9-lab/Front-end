    ...
    <CustomDiv className="bg-[#0B1440] p-6 rounded-xl shadow-2xl border border-slate-700/50 backdrop-blur-sm">
      ...
      <CustomDiv className="flex gap-2 mb-6 overflow-x-auto pb-2">
        {[
          ...
          {
            key: "high",
            label: t("activities.high"),
            count: activities.filter((a) => a.priority === "high").length,
          },
          {
            key: "medium",
            label: t("activities.medium"),
            count: activities.filter((a) => a.priority === "medium").length,
          },
          {
            key: "low",
            label: t("activities.low"),
            count: activities.filter((a) => a.priority === "low").length,
          },
        ].map((filterOption) => (
          <button
            key={filterOption.key}
            onClick={() => setFilter(filterOption.key)}
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all duration-300 whitespace-nowrap flex items-center gap-2 ${
          ...
          >
            ...
            <span
              className={`px-1.5 py-0.5 rounded-full text-xs ${
                filter === filterOption.key ? "bg-white/20" : "bg-slate-600"
              }`}
            >
              {filterOption.count}
            </span>
        ...
        {visibleActivities.map((item) => {
          ...
          return (
            <div
              style={{ width: "95%" }}
              key={item.id}
              className={`group p-4 rounded-lg border-l-4 transition-all duration-300 hover:scale-[1.02] hover:shadow-lg cursor-pointer ${
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <p
                      className={
                        "text-gray-200 text-sm font-medium leading-relaxed transition-colors duration-300 " +
                        (theme == "dark"
                          ? " text-colors-light "
                          : " text-colors-dark ")
                      }
                    ...
                    {timeInfo.badge && (
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-bold ${
                          timeInfo.badge === t("activities.new_badge")
      ...
      {filteredActivities.length > visibleCount && (
        <button
          ...
          className={
            "w-full mt-4 py-2 px-4 bg-slate-800/50 hover:bg-slate-700/50 text-gray-300  rounded-lg text-sm font-medium border-slate-700/50 " +
