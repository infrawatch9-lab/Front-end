    ...
    if (active && payload && payload.length) {
      return (
        <CustomDiv className="bg-[#0B1440] border border-slate-600 rounded-lg p-3 shadow-xl">
          <p className={"text-gray-300 text-sm font-medium mb-2 " + (theme == 'dark' ? " text-colors-light " : " text-colors-dark ")}>{label}</p>
          {payload.map((entry, index) => (
            <CustomDiv key={index} className="flex items-center gap-2 text-sm">
              ...
              />
              <span className={"text-gray-300 " + (theme == 'dark' ? " text-colors-light " : " text-colors-dark ") }>{entry.dataKey === 'blue' ? t('internal.incident') : t('internal.critical')}:</span>
              <span className={"font-medium text-white " + (theme == 'dark' ? " text-colors-light " : " text-colors-dark ")}>{entry.value}</span>
            </CustomDiv>
          ))}
        </CustomDiv>
      );
  ...
  return (
    <CustomDiv className="bg-[#0B1440] border p-4 rounded-lg shadow-lg">
      ...
      <CustomDiv className="flex justify-between items-center mb-4">
        <h2 className={"text-white font-semibold text-sm " + (theme == 'dark' ? " text-colors-light " : " text-colors-dark ")}>{t('internal.incident_history')}</h2>
        <CustomDiv className="flex gap-2 text-xs items-center">
          <button className={"bg-[#162050] text-gray-300 px-3 py-1 rounded text-xs transition-colors " + (theme == 'dark' ? " div-dark-mode-bg " : " div-light-mode-bg ") }>
            {t('internal.all')}
          </button>
          <button className={"bg-[#162050] text-gray-300 px-3 py-1 rounded text-xs transition-colors " + (theme == 'dark' ? " div-dark-mode-bg " : " div-light-mode-bg ") }>
            {t('internal.escalar')}
      ...
      <CustomDiv className="relative">
        ...
        <CustomDiv className="absolute left-0 top-0 h-[200px] flex flex-col justify-between text-xs text-gray-400 -ml-2">
          <span className={ (theme == 'dark' ? " text-colors-light " : " text-colors-dark ") } >100%</span>
          <span className={ (theme == 'dark' ? " text-colors-light " : " text-colors-dark ") } >80%</span>
          <span className={ (theme == 'dark' ? " text-colors-light " : " text-colors-dark ") } >60%</span>
          <span className={ (theme == 'dark' ? " text-colors-light " : " text-colors-dark ") } >40%</span>
          <span className={ (theme == 'dark' ? " text-colors-light " : " text-colors-dark ") } >20%</span>
          <span className={ (theme == 'dark' ? " text-colors-light " : " text-colors-dark ") } >0%</span>
        </CustomDiv>
