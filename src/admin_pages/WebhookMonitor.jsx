...
const WebhookMonitor = () => {
  ...
  return (
    <div className="min-h-screen bg-[#081028] p-4">
      <div className="max-w-full mx-auto">
        ...
        <div className="mb-6">
          ...
          {loading ? (
            <p className="text-gray-400 mt-2">Carregando dados do serviço...</p>
          ) : error ? (
            <p className="text-red-400 mt-2">{error}</p>
          ) : serviceData ? (
            <p className="text-gray-400 mt-2">Real-time message monitoring</p>
          ) : (
            <p className="text-gray-400 mt-2">Real-time message monitoring</p>
          )}
        </div>
        ...
        {loading ? (
          <div className="flex items-center justify-center py-12">
            ...
            <span className="ml-3 text-gray-400">Carregando dados do serviço...</span>
          </div>
        ) : error ? (
          ...
          <div className="space-y-4">{messages.length > 0 ? (
            messages.map((message) => (
              <div 
                key={message.id} 
                className="bg-[#0B1440] border border-[#3B5B75] rounded-lg p-6 hover:bg-[#152055] transition-colors"
              >
                ...
                <div className="bg-[#081028] rounded-lg p-4">
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-gray-300 text-sm">ID: {message.id}</span>
                    </div>
            ...
            <div className="bg-[#0B1440] border border-[#3B5B75] rounded-lg p-12 text-center">
              ...
              <p className="text-gray-300 text-lg mb-2">Waiting for messages...</p>
              <p className="text-gray-500 text-sm">Webhook messages will appear here</p>
            </div>
          )}
          </div>
        )}
