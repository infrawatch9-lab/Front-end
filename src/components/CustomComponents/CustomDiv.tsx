...
const CustomDiv: React.FC<CustomDivProps> =  ( { type, className, children, ...props }, ) => { 
    const { theme, toggleTheme } = useTheme();
    const data = localStorage.getItem("INFRA_WATCH_THEME") || null;

    React.useEffect(() => {
        if (data)
            toggleTheme(data as 'dark' | 'light');
        console.log("HEY!!!!!!") 
        console.log(data);
    }, [ ])
    return (
        <div {...props} className={ className + " " + (theme == 'dark' ? type == 'background' ? "div-dark-mode-bg" : "div-dark-mode-fg" : type == 'background' ? "div-light-mode-bg" : "div-light-mode-fg")}>
            {children}
        </div>
    )
}
